...
def GlobalID_Function(fc, fcMem, GID_bool, sql_N):
    ...
    for s_row in s_GID:
        ...
        z11 = s_row.getValue("lngProjectID")
        z12 = s_row.getValue("SurveyNote")
##        z13 = s_row.getValue("CalcBy")
##        z14 = s_row.getValue("CalcByDate")
...
# multiValue = GetParameterAsText(0) # Add Value from Windows Form Input multivalue # Passed in ass string
##if multiValue.find(mutliValue[0], 'Curb'):
if 'Curb' in multiValue:
    sub_curb = True
##if multiValue.find(mutliValue[0], 'Sidewalk'):
if 'Sidewalk' in multiValue:
    sub_sidewalk = True
##if multiValue.find(mutliValue[0], 'Street'):
if 'Street' in multiValue:
    sub_street = True
##if multiValue.find(mutliValue[0], 'Other'):
##if 'Other' in multiValue:
##    sub_other = True
...
def main():
    """ Create Selection """
    # Need to add the rest of the feature classes from SDE_Base
    # Conditionals for running each feature Type

    ####### Current Error: for this loop. When the loop finds a condition it will load the functions, but when the function returns it needs to return to the next conditional. Not repeat
    ...
    ##    SDE_FC_Copy = arcpy.CopyFeatures_management(SDE_FC_test, scratch + "\\" + "SDE_FC_Copy_Curb")
    ...
    ##    sql_Y = "\"strCCEProjectNumber\"" + " = '" + cceNo + "'"
    ##    mem = r'\in_memory'

    #arcpy.Append_management("mappFC", Sidewalk, "NO_TEST",
    ...
    ##            for row in cursor:
    ##                row.setValue("MeasureType", "Sidewalk")
    ##                cursor.updateRow(row); #print "Sidewalk row updated..."
    ##            del cursor
    ...
    ##            cursor = arcpy.UpdateCursor(fcMem,"","", "MeasureType")
    ##            for row in cursor:
    ##                row.setValue("MeasureType", "SidewalkRamp")
    ##                cursor.updateRow(row); #print "SidewalkRamp row updated..."
    ##            del cursor
    ...
    ##            cursor = arcpy.UpdateCursor(fcMem,"","", "MeasureType")
    ##            for row in cursor:
    ##                row.setValue("MeasureType", "AlleyDriveway")
    ##                cursor.updateRow(row); #print "AlleyDriveway row updated..."
    ##            del cursor
    ...
    ##            cursor = arcpy.UpdateCursor(fcMem,"","", "MeasureType")
    ##            for row in cursor:
    ##                row.setValue("MeasureType", "AlleyPavement")
    ##                cursor.updateRow(row); #print "AlleyPavement row updated..."
    ##            del cursor
        ...
        print fc_BuffList
        ######## Current Error: Need to keep original Global ID values for each record. They do not stay when making a merge or buffer feature class.
        ########                 This is because the feature classes are in a FGDB. Need to figure out how to add the values to the new feature classes.
        ########                    The Global ID values are Unique values needed for the functions later.
        ...
        # Need to delete FC Test Classes
    ##    if arcpy.Exists(scratch + '\\' + "sidewalkTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "sidewalkTEST")
    ##    if arcpy.Exists(scratch + '\\' + "sidewalkOmitTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "sidewalkOmitTEST")
    ##    if arcpy.Exists(scratch + '\\' + "sidewalkRampTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "sidewalkRampTEST")
    ##    if arcpy.Exists(scratch + '\\' + "sidewalkRampOmitTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "sidewalkRampOmitTEST")
    ##    if arcpy.Exists(scratch + '\\' + "sidewalkRampRemoveTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "sidewalkRampRemoveTEST")
    ##    if arcpy.Exists(scratch + '\\' + "additionalPavementTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "additionalPavementTEST")
    ##    if arcpy.Exists(scratch + '\\' + "alleyDrivewayTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "alleyDrivewayTEST")
    ##    if arcpy.Exists(scratch + '\\' + "alleyPavementTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "alleyPavementTEST")
    ##    if arcpy.Exists(scratch + '\\' + "concreteBusPadTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "concreteBusPadTEST")
    ##    if arcpy.Exists(scratch + '\\' + "concretePavementTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "concretePavementTEST")
    ##    if arcpy.Exists(scratch + '\\' + "drivewayRemoveTEST"):
    ##        arcpy.Delete_management(scratch + '\\' + "drivewayRemoveTEST")
...
if __name__ == '__main__':
    main()
##
##while 'Other' in display_List:
####    if item == 'Other':
##    print "Other"
##    GID_bool = False
##    display_List.remove("Other")
##
##    SDE_FC = SDE_Base + '\\' + "Lex_CDOT_Menu_MobileSync.DBO.Dimension"
##    SDE_memory = r"\in_memory"
##    SDE_FC_test = arcpy.MakeFeatureLayer_management(SDE_FC, SDE_memory , sql)
##    SDE_FC_Copy = arcpy.CopyFeatures_management(SDE_FC_test, scratch + "\\" + "SDE_FC_Copy_Other")
##    print "Created Feature Layer..."
##    print "Now loading identical function..."
##
##    identical(SDE_FC_test, target_Display, sql, GID_bool)
